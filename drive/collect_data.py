...
for i in reversed(range(60)):  # warm up camera
    ...
    ret, frame = cam.read()
# create data storage
image_dir = os.path.join(os.path.dirname(sys.path[0]), 'learn/data/', datetime.now().strftime("%Y%m%d%H%M"), 'images/')
    ...
    while True:
        ...
        for e in event.get():
            ...
            if e.type == JOYAXISMOTION:
                ...
                ax4_val = js.get_axis(4)  # -1: max forward, +1: max backward
                # steer
                ...
                if vel > 0:
                    ...
                    engine.stop()
                # record action: [ang_axis, vel_axis]
